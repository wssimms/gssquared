...
void render_line(cpu_state *cpu, int y) {
    ...
    if (ds->display_color_mode == DM_RENDER_MONO) {
        ...
        mono_color_value = { 0xFF, 0xFF, 0xFF, 0xFF }; // override mono color to white when we're in color mode
    }

    if ((ds->display_mode == TEXT_MODE) || ds->display_color_mode == DM_RENDER_MONO) {
