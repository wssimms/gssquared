 ...
 * called page_bits, lores_bits and hires_bits are established which store the
 * current video settings. If page one is selected, page_bits = 0x2000. If page
 * two is selected, page_bits = 0x4000, which in both cases places a set bit at
 * the correct location for forming the address when displaying hires graphics.
 * If hires graphics is selected, hires_bits = 0x7C00, with the set bits corre-
 * sponding to A14-A10, and lores_bits = 0x0000. If hires graphics is not selected,
 * hires_bits = 0x0000, and lores_bits = 0x1C00, with the set bits corresponding
 * to A12-A10. These two variables are masks for the high bits of the address
 * in each mode.
 * 
 ...
 * 
 *     Hires A15-A10 = (page_bits | (vertical-count << 10)) & hires_bits
 *     Lores A15-A10 = ((page_bits >> 3) | HBL) & lores_bits
 * 
 * The variables hires_bits and lores_bits act as masks, ensuring that at all
 * times, one of these values is zero, and the other is the correct high bits
 * of the address scanned by the video system.
 * 
 * To implement mixed mode, two more variables are introduced: mixtxt and
 * mixhgr. If mixed mode is not selected, or if mixed mode is selected but
 * V2 & V4 == 0, then mixtxt = 0x0000 and mixhgr = 0x7C00. If mixed mode is
 * selected and (V2 & V4) == 1, then mixtxt = 0x1C00 and mixhgr = 0x0000.
 * Then the values of the high bits of the scanned address are actually
 * formed like this:
 * 
 *     Hires A15-A10 = (page_bits | (vertical-count << 10)) & (mixhgr & hires_bits)
 *     Lores A15-A10 = ((page_bits >> 3) | HBL) & (mixtxt | lores_bits)
 * 
 * The result of adding the mixtxt and mixhgr terms is that Hires A15-A10
 * is not zero only if both masks mixhgr and hires_bits are nonzero. That is,
 * if both hires mode is selected and if the vertical counter does not specify
 * that text should be scanned. Lores A15-10 is not zero if either mask mixhgr
 * or hires_bits is nonzero. That is, if hires mode is not selected or if hires
 * mode is selected by the vertical counter specifies that text should be scanned.
 *
 * Finally the address to be scanned is formed by summingthe individual parts
